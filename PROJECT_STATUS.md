...
- [x] **ระบบล็อกเอาท์ / ล็อกอินบนอุปกรณ์เคลื่อนที่ (Mobile Navigation Sync):** 
  - เพิ่มปุ่ม "ออกระบบ" (หากล็อกอินอยู่) หรือ "เข้าสู่ระบบ" (หากยังไม่ได้ล็อกอิน) พร้อมไอคอนนำทางที่ด้านล่างขวาของแถบเมนูในระดับ Mobile View (`md:hidden`) เพื่อให้ผู้ใช้บนมือถือสามารถกดจัดการบัญชีและออกจากระบบได้สะดวก
...
- [x] **ระบบลื่นไหลในการสลับหน้า (Instant Page Transitions & Prefetching):** 
  - สร้างไฟล์ `loading.tsx` สเกเลตันโหลดดิ้งพรีเมียม (Skeleton Loader) อัตโนมัติระหว่างที่กำลังดึงข้อมูลฐานข้อมูลบน Server Component ของแดชบอร์ด
  - เปลี่ยนการย้ายหน้าด้วย `router.push('/')` ไปเป็น Next.js `<Link>` เพื่อเปิดฟังก์ชัน prefetching ข้อมูลหน้าหลักล่วงหน้า ทำให้การกดย้ายหน้ากลับไปมาเป็นไปอย่างลื่นไหล ไม่มีอาการหน้าจอค้าง
